...
\noindent \vspace{1mm}
 \textbf{Keywords:} Order Basis, Kernel Basis, Unimodular, Unimodular
Completion
...
parameters \citep{CluzeauQuadrat}.
...
the column basis algorithm from \citep{zl2013}, something which can
be done efficiently. Then the right factor $\mathbf{G}$ can be unimodularly
completed. However, an even simpler way is to apply our algorithm
...
degrees of $\mathbf{A}$. Let $\mathbf{B}\in\mathbb{K}\left[x\right]^{n\times k}$
with $k\in O\left(m\right)$ and the sum $\theta$ of its $\vec{s}$-column
degrees satisfying $\theta\in O\left(ns\right)$. Then we can multiply
...
a degree shift, and $\mathbf{a}\in\mathbb{K}\left[x\right]^{n\times1}$,
a column vector with $\vec{u}$-column degree bounded by $v$. We
define 
\[
\colRev(\mathbf{a},\vec{u},v)=x^{-\vec{u}}\mathbf{a}(x^{-1})~x^{v}=\begin{bmatrix}\rev(p,v-u_{1})\\
\vdots\\
\rev(p,v-u_{n})
\end{bmatrix}.
\]
...
\end{lem}
The following lemmas shows the commutativity between reverse operations
and multiplications. 
\begin{lem}
...
contaiining a reversed $\mathbf{F}$. In addition, \prettyref{lem:nullspaceBasisInOrderBasis}
...
Therefore, Algorithm 2 from \citep{ZL2012} for order basis computation
