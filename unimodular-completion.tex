...
\usepackage[unicode=true,pdfusetitle,
 ...
 {hyperref}
\hypersetup{
 dvips,linkcolor=blue,citecolor=red,urlcolor=green,pdftex=true,hyperindex=true,pdfstartview=FitH,pdfpagelabels=true,colorlinks=true,linkcolor=blue,citecolor=red,urlcolor=green,hypertexnames=false}
\usepackage{breakurl}
...
\title{Efficient Unimodular Completion of Polynomial Matrices}

\author{
Wei Zhou and  George Labahn
         \thanks{
             Cheriton School of Computer Science, University of Waterloo, 
             Waterloo ON, Canada N2L 3G1 \texttt{\{w2zhou,glabahn\}@uwaterloo.ca}
          }
}
...
\maketitle 
\begin{abstract}
Given a matrix $\mathbf{F}\in\mathbb{K}[x]^{m\times n}$ with $m<n$
of univariate polynomials over a field $\mathbb{K}$ we give an efficient
algorithm for computing a unimodular completion of $\mathbf{F}$.
That is, our algorithm finds a polynomial matrix $\mathbf{G}\in\mathbb{K}[x]^{(n-m)\times n}$
such that the square matrix $\left[\begin{array}{c}
\mathbf{F}\\
\mathbf{G}
\end{array}\right]$ is unimodular. Our algorithm is deterministic and computes such a
completion, when it exists, with cost $O^{\sim}\left(n^{\omega}s\right)$
field operations from $\mathbb{K}$,where $s$ is the average of the
$m$ largest column degrees of $\mathbf{F}$. Here $O^{\sim}$ is
big-O but with log factors removed. If a unimodular completion does
not exist for $\mathbf{F}$, our algorithm computes a unimodular completion
for a right cofactor of a column basis of $\mathbf{F}$, or equivalently,
computes a completion that preserves the generalized determinant,
defined as the product of the entries of the Smith normal form.
\end{abstract}

\section{\label{sec:intro}Introduction}

A square matrix of univariate polynomials over a field $\mathbb{K}$
is unimodular if its determinant is a nonzero constant in $\mathbb{K}$.
We consider the unimodular completion 
...
\mathbf{F}\\
\mathbf{G}
\end{array}\right]$ is unimodular. In fact, we consider the more general %completion 
...
In the case of rectangular matrices of multivariate polynomials, the
unimodular completion problem is well known for its role in solving
Serre's conjecture that projective modules over polynomial rings are
free \citet{lam1978}. Serre's conjecture was proved independently
by Quillen and Suslin in 1976 using unimodular completion \citep{YoulaPickel}.
Later on their nonconstructive methods were replaced by constructive
methods, for example that of Logar and Sturmfels \citep{Sturmfels}.
Later extensions include the reduction of mathematical systems to
ones having fewer functions and parameters \citep{CluzeauQuadrat}.
...
of $m=1$ unimodular completion plays a key role in proofs for Hermite
and Smith normal forms.
...
a $2\times2$ unimodular matrix. In the case of $m=1$ so that $\mathbf{F}$
is a single row, a unimodular completion exists if and only if all
the entries in $\mathbf{F}$ are relatively prime \citep{newman1972}.
...
such that 
\begin{equation}
\mathbf{F}\cdot\mathbf{U}=[I_{m},0].\label{one}
\end{equation}
For simplicity and without loss of generality, we focus our discussion
on the situation where the unimodular completion always exists. For
an input matrix that cannot be completed to a unimodular matrices,
we can always factor it as $\mathbf{F}=\mathbf{T}\mathbf{G}$ using
the column basis algorithm from \citep{zl2013}, where the right factor
$\mathbf{G}$ can then be unimodularly completed. This can be done
efficiently. However, an even simpler way is to apply our algorithm
directly on the input matrix, as the algorithm works effectively on
any input matrix to compute a completion that preserves the generalized
determinant.

The algorithm we present in this paper has a a cost of $O^{\sim}\left(n^{\omega}s\right)$
...
an {\em order basis} problem \citep{BeLa94} in a similar way to
that used in the shifted minimal kernel basis algorithm of \citep{za2012}.
In our case we take advantage of the fact that order bases are closely
related to unimodular matrices when one reverses the order of column
coefficients.
...
for our discussion and algorithm. In addition, we discuss the various
ways one can reverse the coefficients of matrix polynomials along
with the properties of such reversals.
...
satisfies $\cdeg_{\vec{t}}\mathbf{B}=\cdeg_{\vec{s}}\left(\mathbf{A}\cdot\mathbf{B}\right)$. 
\end{lem}
...
The following result follows from \citep[Theorem 5.6]{zhou:phd2012}.
The notation $\sum\vec{s}$, for any list $\vec{s}$, denotes the
sum of all entries in $\vec{s}$.
\begin{thm}
...
degrees of $\mathbf{A}$ and $\xi$, a bound on the sum of the entries
of $\vec{s}$. Let $\mathbf{B}\in\mathbb{K}\left[x\right]^{n\times k}$
...
$\mathbf{A}$ and $\mathbf{B}$ with a cost of $O^{\sim}(n^{\omega}s)$.
\end{thm}
...
but restricting it to matrix of polynomials is sufficient for practical
computations, for our purpose in this paper, and for simplicity. Also
note that the order $\vec{\sigma}$ may not be uniform%
\begin{comment}
...
In this paper we will need the following lemma from \citep{zl2013,zhou:phd2012}
which shows a left kernel basis of a right kernel basis is contained
in an order basis of a right kernel basis.
\begin{lem}
\label{lem:nullspaceBasisInOrderBasis} For an input matrix $\mathbf{A}\in\mathbb{K}\left[x\right]^{m\times n}$,
...
We know that vectors whose entries are relatively prime can be completed
to unimodular matrices. We also have a more general criterion on the
existence of unimodular completion for matrices.
\begin{lem}
...
of coefficients of polynomial matrices. Reverse operations has been
...
\end{lem}
The following lemmas shows that the commutativity between reverse
operations and multiplications.
\begin{lem}
...
\end{lem}
More details on these reverse operations can be found in \citep{zhou:phd2012}.
...
\begin{proof}
If $\mathbf{N}$ is a kernel basis of $\mathbf{A}$, then 
\begin{eqnarray*}
\mathbf{A}^{r}\cdot\mathbf{N}^{c} & = & (x^{\vec{a}}\mathbf{A}(x^{-1})x^{\vec{u}})\cdot(x^{-\vec{u}}\mathbf{N}(x^{-1})x^{\vec{b}})\\
 & = & x^{\vec{a}}\cdot\mathbf{A}(x^{-1})\cdot\mathbf{N}(x^{-1})\cdot x^{\vec{b}}=0
\end{eqnarray*}
and so $\mathbf{N}^{c}$ is a kernel basis of $\mathbf{A}^{r}$. It
remains to show minimality. Suppose $\mathbf{N}^{c}$ is not $\vec{u}$-minimal
and we have another kernel basis $\mathbf{M}$ of $\mathbf{A}^{r}$
with $\vec{u}$-column degrees $\vec{c}$ that has some entry lower
than the corresponding entry in $\vec{b}$. Then $\mathbf{M}^{c}=\colRev\left(\mathbf{M},\vec{u},\vec{c}\right)$
...
$\mathbf{F}^{r}$ and, taking tranposes of the rightmost equation
...
that contains a reversed $\mathbf{F}$. In addition, \prettyref{lem:nullspaceBasisInOrderBasis}
...
\begin{lem}
\label{lem:reverseOrderBasisToUnimodular} Let $\vec{u}\in\mathbb{Z}^{n}$
%=\left[u_{1},\dots u_{n}\right]\in\mathbb{Z}^{n}$
be a degree shift and $\mathbf{P}$ a $\left(\mathbf{A},\sigma,\vec{u}\right)$-basis
...
\begin{proof}
Note first that the identity matrix is a $\left(\mathbf{A},0,\vec{u}\right)$-basis,
which has $\vec{u}$-column degrees $\vec{u}$ and determinant $1$.
An $\left(\mathbf{A},\sigma,\vec{u}\right)$-basis $\mathbf{Q}$ can
then be constructed iteratively using the algorithms from \citep{BeLa94,Giorgi2003},
which only increase the $\vec{u}$-column degrees of the basis by
multiplying some columns by $x$ each time. As the $\vec{u}$-column
degrees of the basis are increased from $\vec{u}$ to $\vec{v}$,
its determinant of $\mathbf{Q}$ therefore increased from 1 to $x^{\sum\vec{v}-\sum\vec{u}}$.
Since any two $\left(\mathbf{A},\sigma,\vec{u}\right)$-bases are
unimodularly equivalent, we get 
\[
...
in \prettyref{thm:unimodularComputation} to replace of $\mathbf{F}$
...
basis of the kernel basis can be reversed to provide a uniomodular
...
shifted column $\cdeg_{\vec{u}}\mathbf{M}=\vec{b}$. Let $\mathbf{P}=\left[\mathbf{P}_{1},\mathbf{P}_{2}\right]$
...
\end{array}\right]
\]
a product of two unimodular matrices and hence is itself unimodular. 
\end{proof}
...
and let $\mathbf{M}$ be a $(\mathbf{F}^{r},\vec{u})$-kernel basis
with $\cdeg_{\vec{u}}\mathbf{M}=\vec{b}$ and partitioned as $\mathbf{M}=\left[\mathbf{M}_{1},\mathbf{M}_{2}\right]$.
...
First, the following lemma is a simple extension of Lemma 3.2 in \citet{za2012}
...
\]
 by \prettyref{lem:productDegreeBound}. In addition, we know 
\[
...
It follows that 
\begin{align*}
\sum\cdeg_{-\vec{u}}\mathbf{B}= & \sum\cdeg_{-\vec{u}}\mathbf{P}-\sum\cdeg_{-\vec{u}}\bar{\mathbf{B}}\le\sum\vec{v}-\sum\vec{u}.
\end{align*}

\end{proof}
One can verify that \prettyref{lem:generaKernelBasisDegreeBound}
specializes to \prettyref{thm:boundOfSumOfShiftedDegreesOfKernelBasis}
when $\vec{v}=0$.
...
Partition $\mathbf{P}=\left[\mathbf{P}_{1},\mathbf{P}_{2}\right]$
where $\mathbf{P}_{1}$ consists of all columns $\mathbf{p}$ with
$\cdeg_{-\vec{s}~}\mathbf{p}\le0$. Then $\cdeg_{-\vec{b}-1}\mathbf{M}^{T}\mathbf{P}_{2}=0$
and $\cdeg_{-\vec{s}}\mathbf{P}_{2}=1$.\end{lem}
\begin{proof}
We already know that $\mathbf{P}$ contains a $\left(\mathbf{M}^{T},-\vec{s}\right)$-kernel
basis $\mathbf{P}_{1}$. We know that 
\[
\sum\cdeg_{-\vec{s}}\mathbf{P}=-\sum\vec{s}+\sum\vec{b}+n-m
\]
 as in the proof of \prettyref{lem:generaKernelBasisDegreeBound}
and for the kernel basis $\mathbf{P}_{1}$ in $\mathbf{P}$, we know
\[
\sum\cdeg_{-\vec{s}}\mathbf{P}_{1}=\sum\vec{b}-\sum\vec{s}
\]
 from \prettyref{lem:mutualMinimalNullspaceBasisDegrees}. Therefore,
$\sum\cdeg_{-\vec{s}}\mathbf{P}_{2}=n-m$. From \prettyref{lem:productDegreeBound},
it follows that 
\[
...
given in Algorithm \prettyref{alg:unimodularCompletion}. The situation
here is similar to the situation in computing a left kernel basis
in the column basis computation from \citep{zl2013}. That is, the
order $\vec{b}+1$, or equivalently, the $\vec{s}$-row degrees of
$\mathbf{M}^{T}$ may be unbalanced and can have degree as large as
$\sum\vec{s}$. We therefore follow the same process as in the computation
of column bases \citet{zl2013}. 
...
\[
\mathbf{M}^{T}=\left[\mathbf{M}_{1}^{T},\mathbf{M}_{2}^{T},\cdots,\mathbf{M}_{\left\lceil \log k\right\rceil -1}^{T},\mathbf{M}_{\left\lceil \log k\right\rceil }^{T}\right]
\]
...
basis using Algorithm 2 of \citet{ZL2012}, which can be done with
a cost of $O^{\sim}\left(n^{\omega}s\right)$, where $s=\xi/n$. Partition
...
basis. Set $\vec{s}_{3}=-\cdeg_{-\vec{s}_{2}}\mathbf{N}_{2}$ and
$\tilde{\mathbf{N}}_{2}=\tilde{\mathbf{N}}_{1}\mathbf{N}_{2}.$
...
\]
 Note that $\tilde{\mathbf{N}}_{\left\lceil \log k\right\rceil }$
is a $\left(\mathbf{M},-\vec{s}\right)$-kernel basis. 
\item Let $\mathbf{R}=$ 
\[
...
and $\tilde{\mathbf{N}}_{i-1}\mathbf{Q}_{i}$. We know the kernel
basis computation can be done with a cost of $O^{\sim}\left(n^{\omega}s\right)$
from \prettyref{thm:kerncomp}.
\begin{lem}
...
$2^{i}$ rows. We also have $\sigma_{i}=\left\lceil \xi/2^{i-1}\right\rceil +1\in\Theta\left(\xi/2^{i}\right)$.
Therefore, Algorithm 2 from \citet{ZL2012} for order basis computation
with unbalanced shift can be used with a cost of $O^{\sim}\left(n^{\omega}s\right)$. 
\end{proof}
...
\]
 We also have $\cdeg_{-\vec{s}}\tilde{\mathbf{N}}_{i-1}\le0$, or
equivalently, $\rdeg\tilde{\mathbf{N}}_{i-1}\le\vec{s}$. We can now
use \prettyref{thm:multiplyUnbalancedMatrices} to multiply $\tilde{\mathbf{N}}_{i-1}^{T}$
