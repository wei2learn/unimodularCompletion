...
has $\vec{u}$-column degrees bounded by the corresponding entries
of $\vec{v}$, then $\colRev(\mathbf{A},\vec{u},\vec{v})$ also has
...
if and only if $\colRev\left(\mathbf{N},\vec{u},\cdeg_{\vec{u}}\mathbf{N}\right)$
is a kernel basis of $\mathbf{A}^{r}$. \prettyref{lem:reversedDegreeBound}
then ensures that the minimality also hold at the same time.
\end{proof}
